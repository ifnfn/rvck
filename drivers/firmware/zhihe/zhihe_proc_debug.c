...
static int log_proc_show(struct seq_file *file, void *v)
{
	...
	u32 write = __raw_readl(&log_ctrl->log->rb.write);
	u32 read = __raw_readl(&log_ctrl->log->rb.read);
	u32 size = __raw_readl(&log_ctrl->log->rb.size);
